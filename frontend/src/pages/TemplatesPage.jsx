...
function TemplatesPage() {
  ...
  const visiblePageNumbers = useMemo(
    () => buildPageNumberWindow(page, totalPages),
    [page, totalPages],
  )
          ...
          <>
            ...
            {filteredRows.length > PAGE_SIZE ? (
              <div className="table-pagination">
                <button type="button" className="secondary" onClick={() => setPage((current) => Math.max(1, current - 1))} disabled={page <= 1}>Previous</button>
                <div className="table-pagination-numbers" aria-label="Template Library pages">
                  {visiblePageNumbers[0] > 1 ? (
                    <>
                      <button type="button" className={`secondary${page === 1 ? ' is-active' : ''}`} onClick={() => setPage(1)} aria-current={page === 1 ? 'page' : undefined}>1</button>
                  ...
                  {visiblePageNumbers.map((pageNumber) => (
                    <button
                      key={pageNumber}
                      type="button"
                      className={`secondary${pageNumber === page ? ' is-active' : ''}`}
                  ...
                  {visiblePageNumbers[visiblePageNumbers.length - 1] < totalPages ? (
                    <>
                      ...
                      <button
                        type="button"
                        className={`secondary${page === totalPages ? ' is-active' : ''}`}
                ...
                <button type="button" className="secondary" onClick={() => setPage((current) => Math.min(totalPages, current + 1))} disabled={page >= totalPages}>Next</button>
              </div>
            ) : null}
          </>
        ) : null}
